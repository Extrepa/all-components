...
export function useSVGRenderer() {
    ...
    const renderGridOverlay = useCallback((svg: SVGSVGElement) => {
        // Remove existing grid overlay
        const existingGrid = svg.querySelector('.grid-overlay');
        if (existingGrid) {
            existingGrid.remove();
        }
        ...
        gridGroup.setAttribute('class', 'grid-overlay');
        ...
        const gridColor = isDark ? '#666' : '#ccc';
        ...
        for (let x = minX; x <= minX + width; x += gridSize) {
            ...
            line.setAttribute('y2', String(minY + height));
        ...
        for (let y = minY; y <= minY + height; y += gridSize) {
            ...
            line.setAttribute('x2', String(minX + width));
        ...
        // Insert grid overlay as first child (behind everything)
        if (svg.firstChild) {
    ...
    }, [state.svgElement, state.currentZoom, state.showGridOverlay, applyBackgroundMods, updateSelectionVisual, attachPathListeners, renderGridOverlay, renderMiniMap]);
