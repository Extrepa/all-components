...
const RightPanel: React.FC = () => {
    ...
    // Workflow-based auto-open logic
    useEffect(() => {
        const toolsThatNeedAutoOpen = [
            'transform', 'attributes', 'path-merger', 'boolean-ops', 
            'path-offset', 'color-replacer', 'alignment', 'path-simplifier',
            'text-to-path', 'node-editor'
        ];
        
        if (toolsThatNeedAutoOpen.includes(state.currentPanel)) {
            // Auto-open if tool needs selection and paths are selected, or if tool has parameters
            if (state.selectedPaths.size > 0) {
                setIsCollapsed(false);
                // Also open the appropriate accordion section
                if (['color-replacer', 'path-merger', 'text-to-path', 'path-offset', 'boolean-ops', 'alignment'].includes(state.currentPanel)) {
                    setEditingToolsOpen(true);
                } else if (['path-simplifier'].includes(state.currentPanel)) {
                    setAdvancedToolsOpen(true);
                }
            } else if (['transform', 'attributes'].includes(state.currentPanel)) {
                // Transform and Attributes always need the panel open when selected
                setIsCollapsed(false);
            }
        }
    }, [state.currentPanel, state.selectedPaths]);
