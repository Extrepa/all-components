...
const TransformTool: React.FC = () => {
    ...
    return (
        <div className="tool-panel-content">
            <ToolHeader
                name="Transform Tool"
                whenToUse="After selecting paths or groups, use this to position and adjust them."
                howItWorks="Move, scale, and rotate selected paths or groups using sliders and buttons. Transformations are applied relative to existing transforms."
                description="Transform selected paths or groups by adjusting position, scale, and rotation."
            />

            {selectedCount === 0 ? (
                <div className="tool-status-section">
                    <p style={{ color: 'var(--text-secondary)', marginBottom: '0.75rem', textAlign: 'center' }}>No paths or groups selected</p>
                    <button className="btn btn-primary" onClick={() => updateState({ currentPanel: 'workflow' })} style={{ width: '100%' }}>
