...
const PathOffset: React.FC = () => {
    ...
    return (
        <div className="tool-panel-content">
            <ToolHeader
                name="Path Offset"
                whenToUse="When you need closed paths for cutting machines, or want to merge a stroke with a fill."
                howItWorks="Expands the path outward by the stroke width, creating a new filled path."
                description="Converts strokes into filled shapes by creating an outline around paths. Essential for CNC/laser cutting, stickers (white borders), and merging thick strokes with filled shapes."
            />
            ...
            <div className={`tool-status-section ${selectedCount > 0 ? 'ready' : ''}`}>
                {selectedCount === 0 ? (
                    ...
                    <>
                        ...
                        {selectedPaths.filter((p): p is NonNullable<typeof p> => p !== undefined).slice(0, 3).map(p => (
