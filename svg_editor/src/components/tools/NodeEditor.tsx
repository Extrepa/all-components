...
const NodeEditor: React.FC = () => {
    ...
    return (
        <div className="tool-panel-content">
            <ToolHeader
                name="Node Editor"
                whenToUse="After bridging gaps or merging paths, when you need to adjust specific points. Essential for precision editing."
                howItWorks="Edit individual points (nodes) on paths by clicking and dragging handles. Perfect for fine-tuning curves and fixing bridging issues."
                description="1) Select a path → 2) Click 'Show Nodes' → 3) Drag handles to move points"
            />

            <div className={`tool-status-section ${selectedCount === 1 ? 'ready' : selectedCount > 0 ? 'warning' : ''}`}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <div>
                        <strong style={{ fontSize: '1rem' }}>Editing:</strong>
                        <span style={{ fontSize: '1.25rem', color: selectedCount === 1 ? 'var(--primary-color)' : 'var(--warning-color)', marginLeft: '0.5rem' }}>
                            {selectedCount}
                        </span> path(s)
                    </div>
                    {selectedCount !== 1 && (
                ...
                {selectedCount === 0 && (
                    <p style={{ fontSize: '0.75rem', color: 'var(--warning-color)', marginTop: '0.5rem', fontWeight: 600 }}>
                        Select exactly 1 path in the preview to edit its nodes
                    </p>
                )}
                {selectedCount !== 1 && selectedCount > 0 && (
                    <p style={{ fontSize: '0.75rem', color: 'var(--warning-color)', marginTop: '0.5rem', fontWeight: 600 }}>
                        Please select exactly 1 path to edit nodes
                    </p>
                )}
            </div>
            ...
            {selectedCount === 1 && (
                <div className="tool-parameters">
                    {editingPathId === selectedPathId ? (
                        <>
                            <div style={{ padding: '0.75rem', background: 'rgba(80, 200, 120, 0.1)', borderRadius: 'var(--border-radius)', border: '1px solid var(--success-color)', marginBottom: '1rem' }}>
                                <p style={{ fontSize: '0.875rem', color: 'var(--success-color)', margin: 0 }}>
