/**
 * LoadingScreen - Enhanced loading screen overlay with Errl Club neon theme
 */
export class LoadingScreen {
    ...
    create() {
        ...
        // Ready button (initially unfilled/disabled, says "PREPARING GAME...")
        // This button should be smaller/thinner than the loading bar
        ...
        portalButton.id = 'loading-portal-button';
    ...
    handleReady() {
        ...
        // The loading screen will be hidden by the game once it's confirmed running
    }
    ...
    updateProgress(progress, message = '') {
        ...
        if (this.progressBar) {
            ...
            if (fill) {
                ...
                // Ensure width is set to exactly the percentage
                ...
                // Add completion animation when progress reaches 100% (only once)
                if (percentage >= 100 && !fill.classList.contains('completed')) {
                    fill.classList.add('completed');
                    // Combine shimmer and completion animations
                    fill.style.animation =
                        'loadingShimmer 2s linear infinite, progressComplete 1s ease-in-out infinite';
                }
            }
        }

        // Use currentProgress instead of progress to ensure we check the actual state
        const currentPercent = this.currentProgress * 100;

        // Show progress text for all stages until 100%
        if (this.progressText) {
            if (currentPercent < 100 && message && this.currentProgress >= progress) {
                // Show progress text during loading (only if not downgrading)
                ...
                // Hide progress text at 100% when button appears
                this.progressText.style.transition = 'opacity 0.3s ease-in-out';
                this.progressText.style.opacity = '0';
                // Clear text to ensure it doesn't show old message
                setTimeout(() => {
                    if (this.progressText) {
        ...
        // When progress reaches 0.9+, show button (still showing "PREPARING GAME..." until 100%)
        if (this.currentProgress >= 0.9 && this.readyButton && !this.isReady) {
            this.readyButton.style.opacity = '1';
        }

        // Show ready button when progress reaches 100% (only once)
        if (this.currentProgress >= 1.0 && this.readyButton && !this.isReady) {
            this.showReadyButton();
            // Wait for user to click "Ready to Enter Game" - don't auto-hide
        }
    }

    /**
     * Show ready button (fill it and enable it, change text to "READY?")
     */
    showReadyButton() {
        if (this.isReady) {
            return;
        ...
        // Fill it with gradient and enable it, change text to "READY?"
        setTimeout(() => {
            if (this.readyButton) {
                // Change text to "READY TO ENTER GAME" with smooth transition
                this.readyButton.style.transition = 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)';
                this.readyButton.textContent = 'READY TO ENTER GAME';
                ...
                // Increase button width to fit longer text (60% instead of 50%)
                this.readyButton.style.width = '60%';
                this.readyButton.style.minWidth = '300px';
                // Keep the same padding and size (chunkier)
                this.readyButton.style.padding = '12px 24px';
                this.readyButton.style.fontSize = 'clamp(0.85rem, 1.4vw, 1.05rem)';
                this.readyButton.style.fontFamily =
                    "'Arial Black', 'Arial Bold', 'Impact', sans-serif";

                // Fill it with gradient and enable it (smooth transition)
                this.readyButton.style.color = '#000000';
                this.readyButton.style.background =
                    'linear-gradient(135deg, #00ffff 0%, #ff00ff 100%)';
                this.readyButton.style.backgroundSize = '200% 100%';
                this.readyButton.style.border = '1px solid #00ffff';
                ...
                this.readyButton.setAttribute('aria-label', 'Ready to start game');
    ...
    showError(error) {
        ...
        let errorContainer = document.getElementById('loading-error-container');
        if (!errorContainer) {
            errorContainer = document.createElement('div');
            errorContainer.id = 'loading-error-container';
    ...
    /**
     * Hide loading screen with enhanced fade out
     */
    hide() {
        ...
        if (!this.container) {
            ...
            if (this.container) {
                console.log('LoadingScreen: Recovered container from DOM by ID');
            }
        }
        ...
        if (this.container) {
            ...
            setTimeout(() => {
                if (this.container) {
                    ...
                    console.log('LoadingScreen: Fade complete, removing from DOM');
                    this.remove();
                }
            }, 800);
            ...
            setTimeout(() => {
                ...
                if (loadingScreenElement && loadingScreenElement.parentNode) {
                    console.warn(
                        'LoadingScreen: Fallback removal triggered - screen was still visible'
                    );
                    loadingScreenElement.style.display = 'none';
                    loadingScreenElement.style.pointerEvents = 'none';
                    loadingScreenElement.parentNode.removeChild(loadingScreenElement);
                }
            }, 2000);
        } else {
            console.warn('LoadingScreen: hide() called but container is null');
        }
    }
    ...
    remove() {
        ...
        const portalButton = document.getElementById('loading-portal-button');
