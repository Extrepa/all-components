/**
 * ErrlLoader - Animated walking Errl loader component
 *
 * Based on the original animated SVG loader with authentic Errl parts
 */
export class ErrlLoader {
    /**
     * Create a new ErrlLoader
     * @param {Object} config - Configuration
     * @param {HTMLElement} config.container - Container element (optional, defaults to body)
     * @param {string} config.message - Loading message (optional)
     */
    constructor(config = {}) {
    ...
    createLoader() {
        ...
        this.element.id = 'errl-loader-container';
        this.element.style.cssText = `
            ...
            width: 100%;
            height: 100%;
            background: radial-gradient(circle at 30% 30%, #0a0012 0%, #1a0033 70%);
            display: flex;
        ...
        loaderWrapper.className = 'errl-loader';
        ...
        loaderWrapper.appendChild(head);

        // Create body
        const body = this.createBody();
        loaderWrapper.appendChild(body);

        // Create arms
        const leftArm = this.createArm('left');
        loaderWrapper.appendChild(leftArm);
        const rightArm = this.createArm('right');
        loaderWrapper.appendChild(rightArm);

        // Create legs
        const leftLeg = this.createLeg('left');
        loaderWrapper.appendChild(leftLeg);
        const rightLeg = this.createLeg('right');
        loaderWrapper.appendChild(rightLeg);

        this.element.appendChild(loaderWrapper);
    ...
    createHead() {
        ...
        skull.setAttribute('cx', '67.5');
        skull.setAttribute('cy', '67.5');
        skull.setAttribute('r', '35');
        skull.setAttribute('fill', '#ffffff');
        skull.setAttribute('stroke', '#000000');
        skull.setAttribute('stroke-width', '2');
        svg.appendChild(skull);

        // Face patch
        const facePatch = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        facePatch.setAttribute('cx', '67.5');
        facePatch.setAttribute('cy', '67.5');
        facePatch.setAttribute('r', '32');
        facePatch.setAttribute('fill', '#ffffff');
        facePatch.setAttribute('stroke', '#000000');
        facePatch.setAttribute('stroke-width', '0');
        svg.appendChild(facePatch);

        // Facial features group
        const featuresGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        featuresGroup.setAttribute('transform', 'translate(7,11)');

        const innerGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        innerGroup.setAttribute('transform', 'translate(26,10.1079) scale(0.065776)');
        innerGroup.setAttribute('fill', '#000000');
        innerGroup.setAttribute('stroke', '#e5eef7');
        innerGroup.setAttribute('stroke-width', '1');
        innerGroup.setAttribute('vector-effect', 'non-scaling-stroke');
        innerGroup.setAttribute('stroke-linejoin', 'round');
        innerGroup.setAttribute('stroke-linecap', 'round');
        ...
        innerGroup.appendChild(mouth);

        featuresGroup.appendChild(innerGroup);
        svg.appendChild(featuresGroup);
    ...
    createBody() {
        ...
        ellipse.setAttribute('cx', '65');
        ellipse.setAttribute('cy', '65');
        ellipse.setAttribute('rx', '60');
        ellipse.setAttribute('ry', '62');
        ellipse.setAttribute('fill', '#8427F5');
    ...
    createArm(side) {
        ...
        arm.className = `errl-arm-${side}`;

        const isLeft = side === 'left';
        arm.style.cssText = `
            position: absolute;
            width: 32px;
            height: ${isLeft ? '200px' : '180px'};
            top: 70px;
            ${isLeft ? 'left: 47px;' : 'right: 50px;'}
            z-index: 3;
            transform-origin: ${isLeft ? '10px 15px' : '20px 20px'};
            animation: armSwing 2.0s infinite ease-in-out;
            ${isLeft ? 'animation-delay: 1.0s;' : ''}
        ...
        if (isLeft) {
            svg.setAttribute('viewBox', '103.6 1227.3039073806078 149.48 395.2836468885673');
            const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            path.setAttribute(
                'd',
                'M 103.60 1227.30 L103.60 1555.97 L106.56 1570.77 L119.88 1595.94 L140.60 1613.70 L165.76 1622.59 L190.92 1622.59 L210.16 1616.67 L227.92 1604.82 L239.76 1592.98 L251.60 1564.85 L253.08 1227.30 Z'
            );
            path.setAttribute('fill', '#8427F5');
            path.setAttribute('stroke', '#0b0d1a');
            ...
            svg.setAttribute(
                'viewBox',
                '1136.6399999999999 1227.3039073806078 127.2800000000002 485.5918958031839'
            );
            const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            path.setAttribute(
                'd',
                'M 1136.64 1227.30 L1136.64 1578.17 L1139.60 1664.04 L1142.56 1674.40 L1149.96 1686.25 L1166.24 1702.53 L1179.56 1709.93 L1207.68 1712.90 L1221.00 1709.93 L1235.80 1702.53 L1253.56 1683.29 L1260.96 1668.48 L1262.44 1658.12 L1263.92 1227.30 Z'
            );
            path.setAttribute('fill', '#8427F5');
            path.setAttribute('stroke', '#0b0d1a');
            path.setAttribute('stroke-width', '13');
    ...
    createLeg(side) {
        ...
        leg.style.cssText = `
            ...
            height: 100px;
            top: 142px;
            ${isLeft ? 'left: 55px;' : 'right: 55px;'}
            z-index: 2;
            transform-origin: 20px 15px;
            filter: drop-shadow(0 4px 4px rgba(0,0,0,0.8));
            animation: ${isLeft ? 'legStepL' : 'legStepR'} 1s infinite ease-in;
            ${!isLeft ? 'animation-delay: 0.5s;' : ''}
        `;
        ...
        if (isLeft) {
            svg.setAttribute('viewBox', '293.04 1227.3039073806078 281.2 787.6063675832129');
            const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            path.setAttribute(
                'd',
                'M 293.04 1227.30 L293.04 1561.89 L298.96 1894.99 L301.92 1917.20 L319.68 1954.21 L328.56 1966.05 L353.72 1989.74 L390.72 2008.99 L418.84 2014.91 L452.88 2014.91 L467.68 2011.95 L489.88 2004.55 L516.52 1989.74 L541.68 1966.05 L560.92 1937.93 L572.76 1900.91 L574.24 1227.30 Z'
            );
            path.setAttribute('fill', '#8427F5');
            path.setAttribute('stroke', '#0b0d1a');
            ...
            svg.setAttribute(
                'viewBox',
                '871.72 1227.3039073806078 227.92000000000007 818.6960926193922'
            );
            const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            path.setAttribute(
                'd',
                'M 871.72 1227.30 L871.72 1678.85 L874.68 1766.19 L874.68 1912.76 L877.64 1967.54 L883.56 1985.30 L890.96 1998.63 L905.76 2016.39 L922.04 2029.71 L939.80 2038.60 L962.00 2044.52 L1031.56 2046.00 L1058.20 2032.68 L1075.96 2013.43 L1090.76 1982.34 L1096.68 1958.65 L1099.64 1769.15 L1099.64 1227.30 Z'
            );
            path.setAttribute('fill', '#8427F5');
            path.setAttribute('stroke', '#0b0d1a');
            path.setAttribute('stroke-width', '15');
    ...
    addAnimations() {
        ...
        if (document.getElementById('errl-loader-styles')) {
            return;
        }

        const style = document.createElement('style');
        style.id = 'errl-loader-styles';
        style.textContent = `
            @keyframes headBob {
            ...
            @keyframes legStepL {
                ...
                20%  { transform: rotate(25deg) translate(10px,-12px); }
                40%  { transform: rotate(0deg) translate(5px,0px); }
                60%  { transform: rotate(-20deg) translate(-5px,2px); }
                100% { transform: rotate(-10deg) translate(0px,0px); }
            }
            @keyframes legStepR {
                0%   { transform: rotate(10deg) translate(0px,0px); }
                20%  { transform: rotate(-25deg) translate(-10px,-10px); }
                40%  { transform: rotate(0deg) translate(-5px,0px); }
                60%  { transform: rotate(20deg) translate(5px,2px); }
                100% { transform: rotate(10deg) translate(0px,0px); }
            }
    ...
    dispose() {
        ...
        const style = document.getElementById('errl-loader-styles');
        if (style && !document.querySelector('.errl-loader')) {
    ...
    setMessage(message) {
        ...
        if (this.element) {
            const messageEl = this.element.querySelector('.errl-loader-message');
            if (messageEl) {
                messageEl.textContent = message;
            } else if (message) {
                const newMessageEl = document.createElement('div');
                newMessageEl.className = 'errl-loader-message';
