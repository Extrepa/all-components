...
import { ThemeMode, EffectMode, NeonColor } from '../designSystem/errlDesignSystem';
...
export function ThemeControls({ componentId, showGlobalToggle = true, compact = false }: ThemeControlsProps) {
  ...
  const handleThemeChange = (newTheme: ThemeMode) => {
    if (componentId && !isGlobal) {
      setComponentOverride?.(componentId, { theme: newTheme });
    } else {
      setTheme(newTheme);
    }
  };
  ...
  const handleEffectChange = (newEffect: EffectMode) => {
    if (componentId && !isGlobal) {
      setComponentOverride?.(componentId, { effect: newEffect });
    } else {
      setEffect(newEffect);
    }
  };
  ...
  const handleNeonColorChange = (newColor: NeonColor) => {
    if (componentId && !isGlobal) {
      setComponentOverride?.(componentId, { neonColor: newColor });
    } else {
  ...
  const handleGlobalToggle = (global: boolean) => {
    ...
    if (componentId && global) {
      setComponentOverride?.(componentId, null);
    }
  };
