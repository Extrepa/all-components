...
export const ErrlEditor: React.FC = () => {
  ...
  const handleSaveProject = () => {
    ...
    a.href = url;
    a.download = `errl-project-${Date.now()}.json`;
    a.click();
  ...
  const handleExportPNG = useCallback(async () => {
    ...
    try {
      ...
      const dataUrl = await exportSvgElementAsPng(svg, {
        width: scene.viewport.width,
        height: scene.viewport.height,
        backgroundColor: "#09090b",
      });
      const link = document.createElement("a");
      link.download = `errl-scene-${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleExportSVG = useCallback(() => {
    ...
    const svgString = exportSvgElementAsCleanSvg(svg, {
      backgroundColor: "#09090b",
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `errl-scene-${Date.now()}.svg`;
    a.click();
    URL.revokeObjectURL(url);
  }, []);
  ...
  const handleExportPreset = useCallback(async (preset: { id: string; width: number; height: number; transparentBg?: boolean }) => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.download = `errl-scene-${preset.id.toLowerCase()}-${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
