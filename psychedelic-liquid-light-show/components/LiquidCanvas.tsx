...
import { Dropper3D } from './Dropper3D';
...
const Canvas2DFluid = (canvas: HTMLCanvasElement, initialConfig: LiquidConfig, opts?: { performanceMode?: boolean }) => {
  ...
  const instance = {
    ...
    splat: (x: number, y: number, radius: number, color: string, reactionStrength: number = 1.0) => {
      const { devicePixelRatio = 1 } = window;
      ...
      // Apply reaction strength: smaller drops = more particles and higher velocity
      // Keep it very concentrated - particles should stay close together
      const baseNumParticles = perf ? 3 : 5;
      const numParticles = Math.round(baseNumParticles * Math.min(reactionStrength, 1.5)); // Lower cap for particle count
      const baseSpeed = 3;
      const speedMultiplier = Math.min(reactionStrength, 1.2); // Lower speed multiplier

      for (let i = 0; i < numParticles; i++) {
        const angle = Math.random() * Math.PI * 2;
        const speed = (Math.random() * baseSpeed + 1) * speedMultiplier;
        // Keep visual radius very concentrated - minimal expansion
        const visualRadius = radius * (1 + (reactionStrength - 1) * 0.1); // Very minimal expansion (0.1)
        const particle = {
            x: x * devicePixelRatio,
            y: y * devicePixelRatio,
            radius: (visualRadius + (Math.random() - 0.5) * visualRadius * 0.5) * devicePixelRatio,
            color,
            life: maxLife,
            maxLife: maxLife,
...
export const LiquidCanvas: React.FC<LiquidCanvasProps> = ({ config, isPlaying, activeColorIndex, setGetDataUrlCallback, setGetStreamCallback, setClearCallback, cursorUrl, isDemoMode, onDemoEnd, cycleEnabled = false, cycleMode = 'sequential', cycleCadence = 'per-stroke', selectedPresets = [], presets = [], onCommitConfig, performanceMode, setSymmetryOriginMode, updateConfig, onChangeActiveColor }) => {
  ...
  const [normalizedDropSize, setNormalizedDropSize] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Update normalized drop size for 3D dropper (0 to 1)
      setNormalizedDropSize(eased);
      
      // Always hide preview circle when dropper is enabled (3D dropper provides visual feedback)
      if (configRef.current.dropperEnabled) {
        setPreviewCircle(null);
      } else if (configRef.current.dropPreview) {
        // Only show preview circle as fallback when dropper is disabled
        const minR = configRef.current.dropMinRadius || 0.01;
        const maxR = configRef.current.dropMaxRadius || 0.25;
        const canvasWidth = containerRef.current?.clientWidth || 800;
  ...
  const performSplat = useCallback((x: number, y: number, radiusPx: number) => {
    ...
    // Calculate inverse reaction strength based on drop size
    // Smaller drops = bigger reactions, but much more controlled and concentrated
    const minRadius = (currentConfig.dropMinRadius || 0.01) * canvasWidth;
    const maxRadius = (currentConfig.dropMaxRadius || 0.25) * canvasWidth;
    const normalizedSize = (radiusPx - minRadius) / (maxRadius - minRadius); // 0 to 1
    const reactionMultiplier = currentConfig.dropReactionMultiplier || 2.0;
    // Inverse relationship: smaller drops (low normalizedSize) = higher reaction strength
    // Use a much gentler curve: clamp to tighter range (0.8 to 2.0x) for more concentrated drops
    const rawReaction = reactionMultiplier * (1 / (normalizedSize + 0.5)); // +0.5 for much gentler curve
    const reactionStrength = Math.max(0.8, Math.min(2.0, rawReaction)); // Cap between 0.8x and 2.0x (tighter range)
    ...
    points.forEach(point => {
      ...
      if (rendererRef.current) {
        rendererRef.current.splat(px, py, radiusPx, color, phase, reactionStrength);
      } else if (fallbackFluidRef.current) {
        fallbackFluidRef.current.splat(px, py, radiusPx, color, reactionStrength);
      }
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (currentConfig.dropperEnabled && !currentConfig.lineEnabled && !currentConfig.dripEnabled && pointerDownAtRef.current) {
      ...
      const minR = currentConfig.dropMinRadius || 0.01;
      const maxR = currentConfig.dropMaxRadius || 0.25;
      const canvasWidth = containerRef.current?.clientWidth || 800;
      const radiusPx = lerp(minR * canvasWidth, maxR * canvasWidth, eased);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Update cursor position for 3D dropper (screen coordinates) - always track when over canvas
    const currentConfig = configRef.current;
    if (currentConfig.dropperEnabled && !currentConfig.lineEnabled && !currentConfig.dripEnabled) {
      setCursorPosition({ x: e.clientX, y: e.clientY });
    }
  ...
  const handlePointerEnter = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    // Initialize cursor position when entering canvas for 3D dropper
    const currentConfig = configRef.current;
    if (currentConfig.dropperEnabled && !currentConfig.lineEnabled && !currentConfig.dripEnabled) {
      setCursorPosition({ x: e.clientX, y: e.clientY });
    }
  ...
  return (
    ...
    >
      ...
      {/* Preview circle overlay - red transparent with grid lines (completely disabled when dropper is enabled) */}
      {previewCircle && !config.dropperEnabled && !config.lineEnabled && !config.dripEnabled && (
        <div
      ...
      {config.dropperEnabled && !config.lineEnabled && !config.dripEnabled && (
        <Dropper3D
          ...
          dropSize={normalizedDropSize}
          phase={config.activePhase || 'oil'}
