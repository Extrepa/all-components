            ...
            <>
              ...
              <Tooltip text="Minimum drop size (quick tap)">
                <Slider 
                  ...
                  onChange={v => updateConfig({ dropMinRadius: Math.min(v, config.dropMaxRadius || 0.25) })} 
                  min={0.005}
                  max={0.15}
                  step={0.005}
                />
              ...
              <Tooltip text="Maximum drop size (hold 1.2s)">
                <Slider 
                  label={`Max Size: ${((config.dropMaxRadius || 0.25) * 100).toFixed(0)}%`}
                  value={config.dropMaxRadius || 0.25} 
              ...
              <Tooltip text="Inverse reaction strength: smaller drops create bigger reactions">
                <Slider 
                  label={`Reaction Multiplier: ${(config.dropReactionMultiplier || 2.0).toFixed(1)}x`}
                  value={config.dropReactionMultiplier || 2.0} 
                  onChange={v => updateConfig({ dropReactionMultiplier: v })} 
                  min={0.5}
                  max={5.0}
                  step={0.1}
                />
