...
export class LiquidRenderer {
   ...
   * @param reactionStrength - Inverse relationship: smaller drops = bigger reactions (default 1.0)
   */
  public splat(x: number, y: number, radius: number, color: string, phase: 'oil' | 'water', reactionStrength: number = 1.0) {
    ...
    // Apply reaction strength to visual radius: keep it more concentrated
    // Reduced from 0.5 to 0.15 for tighter, more focused drops
    const visualRadius = radius * (1 + (reactionStrength - 1) * 0.15);
    const normRadius = (visualRadius / this.app.renderer.width) * 2.0;
    
    // Apply reaction strength to thickness: smaller drops = more force, but capped
    const baseThickness = 1.5;
    const thickness = baseThickness * Math.min(reactionStrength, 2.0); // Cap at 2.0x
