...
import {setFeed, setScreensaverMode, setScreensaverSound} from '../lib/actions.ts'
import {initializeAudio, playSound} from '../lib/useTonePalette.ts'
import type {Output, Round} from '../lib/types.ts'
import models from '../lib/models.ts'
import {scrollToPosition} from '../lib/utils.ts'
...
export function Screensaver() {
  ...
  const flattenedFeed = useMemo(() => {
    if (!feed) return []
    return flatten(feed.map(round => Object.values(round.outputs))) as Output[]
  }, [feed])
      ...
      {items.map((item, index) => {
        if (item) {
          ...
          const activeRound = feed!.find(round =>
            Object.values(round.outputs).some((output: Output) => output.id === item)
          )!
          return (
            <div
              key={index}
